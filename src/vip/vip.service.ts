...
export class VipService {
  ...
  async getVipList(): Promise<xVip_vips[]> {
    return await this.prisma.xVip_vips.findMany();
  }

  async getLogs(): Promise<xVip_logs[]> {
    return await this.prisma.xVip_logs.findMany({
      orderBy: {
        timestamp: 'desc',
      },
    });
  ...
  async createVip(vip: xVip_vips): Promise<xVip_vips> {
    ...
    if (existingVip) {
      throw new ConflictException(`vip already exists for ${vip.steamid}`);
    }

    return await this.prisma.xVip_vips.create({ data: vip });
  }

  async createLog(log: xVip_logs): Promise<xVip_logs> {
    try {
      return await this.prisma.xVip_logs.create({ data: log });
    } catch (e) {
  ...
  async updateVip(vip: xVip_vips): Promise<xVip_vips> {
    ...
    return await this.prisma.xVip_vips.update({
      where: {
        steamid: vip.steamid,
      },
      data: vip,
  ...
  async deleteVip(vip: xVip_vips): Promise<xVip_vips> {
    ...
    return await this.prisma.xVip_vips.delete({
      where: {
        steamid: vip.steamid,
      },
    });
