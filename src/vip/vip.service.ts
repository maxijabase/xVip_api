...
export class VipService {
  ...
  async createVip(vip: xVip_vips): Promise<xVip_vips> {
    ...
    if (existingVip) {
      throw new HttpException(`vip already exists for ${vip.steamid}`, HttpStatus.CONFLICT);
    }
  ...
  async updateVip(vip: xVip_vips): Promise<xVip_vips> {
    ...
    if (!existingVip) {
      throw new HttpException(`vip does not exist for ${vip.steamid}`, HttpStatus.NOT_FOUND);
    }
